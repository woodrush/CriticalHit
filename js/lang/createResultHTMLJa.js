function createResultHTML(resultSlot, res){
  ...
  + '<tr>'
  + '<br>'
  + '<b>' + moveNames[damageResults[resultSlot].moveId] + '</b>&nbsp;<img src="./image/' + (damageResults[resultSlot].aCategory == 1 ? 'physical' : 'special') + '.png"  alt="' + (damageResults[resultSlot].aCategory == 1 ? '物理' : '特殊') + '">'
  + '&nbsp;<span class="label" style="background:' + typeSchemes[damageResults[resultSlot].moveType] + '">威力' + damageResults[resultSlot].basePower + '/' + typeNames[damageResults[resultSlot].moveType] + '</span><br>'
//  + '<br>'
  + (damageResults[resultSlot].aCategory == 1 ? '攻撃' : '特攻') + damageResults[resultSlot].stats[0] + ' → ' + (damageResults[resultSlot].dCategory == 1 ? '防御' : '特防') + damageResults[resultSlot].stats[1] + ' HP' + damageResults[resultSlot].stats[2]
//  + '　天候:' + weatherNames[weather] + '<br>'
  + '<br>'
//  + '倍率:1*1.5<br>'
//  + ' <span class="label label-warning">確定2</span> (82.8%〜97.7%)<br>'
  + '<b>' + res[0] + '-' + res[15] + '</b> (' + lowerBound + '%〜' + upperBound + '%)'
  + '&nbsp;<span class="label label-inverse" style="cursor:pointer" onClick="alert(\'ダメージ列:\\n' + res + '\');">詳細</span><br>'
  + '</td>'
  + '</tr>'
  + '<tr>'
  + '<td>'
